...
def extract_players(data):
    assert "players" in data, "[M05] ERROR: 'players' key missing"
    players = data["players"]
    assert isinstance(players, dict), "[M05] ERROR: 'players' must be a dict"
    return players
    ...
    for file in m05_files:
        ...
        for persona, stats in players.items():
            ...
            for key in ["max_kills", "max_deaths", "max_assists"]:
                ...
                for subkey in entry:
                    assert subkey in ["kills", "deaths", "assists", "match_id"], f"[M05] ERROR file={rel}: invalid key {subkey}"
...
def test_m05_match_id_validity():
    ...
    for file in m05_files:
        ...
        for persona, stats in players.items():
            for key in ["max_kills", "max_deaths", "max_assists"]:
                entry = stats[key]
                assert entry["match_id"] > 0, f"[M05] ERROR file={rel}: invalid match_id"
