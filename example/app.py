...
from utils.pipeline_status_manager import get_pipeline_status, set_pipeline_status
...
def create_app() -> Dash:
    ...
    app.layout = html.Div([
        ...
        dcc.Interval(id="pipeline-status-refresh", interval=2000, n_intervals=0),  # Check pipeline status every 2 seconds
        dcc.Interval(id="log-refresh", interval=2000, n_intervals=0),  # Refresh logs every 2 seconds
        ...
        html.Div(id="header-container", className="header-container", children=[
            ...
            html.Div(className="header-right", children=[
                ...
                html.Div(id="pipeline-status-indicator", children=[]),
    ...
    # Callback to update pipeline status indicator (reads from file, shared across all users)
    @app.callback(
        Output("pipeline-status-indicator", "children"),
        Input("pipeline-status-refresh", "n_intervals"),
    )
    def update_pipeline_status_indicator(_):
        status = get_pipeline_status()
        
        if status == "RUNNING":
            return html.Div([
                html.Div(className="spinner"),
                html.Span("Ejecutando Pipeline...")
            ], className="pipeline-status-indicator running")
        elif status == "SUCCESS":
            return html.Div([
                html.Span("✓", className="status-icon"),
                html.Span("Completado")
            ], className="pipeline-status-indicator success")
        elif status == "ERROR":
            return html.Div([
                html.Span("✗", className="status-icon"),
                html.Span("Error")
            ], className="pipeline-status-indicator error")
        else:  # IDLE or None
            return []
    ...
    def update_tab_content(selected_tab, pool_id, min_friends, report_id, is_season_mode):
        ...
        if is_season_mode:
            # SEASON MODE: ALWAYS use season date range
            start_date = SEASON_START_DATE
            end_date = SEASON_END_DATE
        else:
            # NORMAL MODE: only set dates if report_id specifies them
        ...
        # Determine Last Updated Text with Pool Name and Formatted Date
        last_updated_text = ""
        try:
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    if ts:
                        ...
                        try:
                            ...
                            formatted_date = ts[:16]  # Fallback
                        
                        # Determine display name for pool
                        if is_season_mode:
                            pool_display = "Season 2026"
                        else:
                            pool_display = f"Pool {pool_id[:8]}" if pool_id and pool_id != "auto" else "Pool"
                        
                        last_updated_text = f"Última actualización: [{pool_display}] - {formatted_date}"
        except Exception:
            pass
    ...
    def execute_pipeline_cb(n_clicks, pool_id, min_friends, is_season_mode):
        ...
        # Set status to RUNNING (visible to all users)
        ...
        def worker():
            try:
                ...
                PIPELINE_QUEUE.put("[PIPELINE] ✅ Pipeline completado exitosamente.\n")
            except Exception as e:
                # Pipeline error
                set_pipeline_status("ERROR")
